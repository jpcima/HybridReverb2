    ...
    : master (m)
{
    ...
    paramEnvelope.reset(new ParamEnvelope());

    numPlotPoints = 1 + 14 * IRPLOT_GRID_X,
    plotMax.reset(new float[numPlotPoints]);
    plotRMS.reset(new float[numPlotPoints]);
    plotRMSAccu.reset(new float[numPlotPoints]);
    plotRMSCounter.reset(new int[numPlotPoints]);
