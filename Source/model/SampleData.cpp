...
int SampleData::applyLoadFiles(const String &dir,
                               const String &fileLL,
                               const String &fileLR,
                               const String &fileRL,
                               const String &fileRR,
                               std::vector<String> *errors)
{
    ...
    {
        const String &filebase = *filenames[i];
        if (filebase.isEmpty())
            available[i] = false;
        else
        {
            String fullpath = (File::isAbsolutePath(filebase)) ?
                filebase : (dir + filebase);
            if (loadSingleFile(fullpath, i, &error) != 0)
            {
                if (errors)
...
int SampleData::applyLoadFiles(String dir, ParamImpulseResponses *param,
                               std::vector<String> *errors)
{
    return applyLoadFiles(dir,
                          param->ll,
                          param->lr,
                          param->rl,
                          param->rr,
...
int SampleData::loadSingleFile(String filename, int num, String *error)
{
    ...
    {
        available[num] = false;
        String message = TRANS("Error reading audio file") + " \"" + filename + "\"";
        if (error)
            *error = message;
        else
            fprintf(stderr, "%s\n", message.toRawUTF8());
    ...
    std::unique_ptr<int[]> readBuffer(new int[reader->lengthInSamples]);
    int *readChannels[] = {readBuffer.get()};
    ...
    {
    case 0:
        sampleData = samplesLL.get();
        break;
    case 1:
        sampleData = samplesLR.get();
        break;
    case 2:
        sampleData = samplesRL.get();
        break;
    case 3:
        sampleData = samplesRR.get();
        break;
    }
    ...
    {
        memcpy(sampleData, readBuffer.get(), dataLen * sizeof(float));
    }
    else
    {
...
void SampleData::applyTimbre(SampleData *source, int filterLen, float *filter)
{
    ...
    if (sLL != 0)
        applySingleTimbre(&hcSingle, sLL, samplesLL.get(), dataLen);

    if (sLR != 0)
        applySingleTimbre(&hcSingle, sLR, samplesLR.get(), dataLen);

    if (sRL != 0)
        applySingleTimbre(&hcSingle, sRL, samplesRL.get(), dataLen);

    if (sRR != 0)
        applySingleTimbre(&hcSingle, sRR, samplesRR.get(), dataLen);
...
float *SampleData::getData(int num)
{
    ...
    {
    case 0:
        return samplesLL.get();
    case 1:
        return samplesLR.get();
    case 2:
        return samplesRL.get();
    case 3:
        return samplesRR.get();
    }
    return 0;
}
...
void SampleData::checkLength(int len)
{
    ...
    newBuf.reset(new float[bufferLen]);
    if (samplesLL)
        memcpy(newBuf.get(), samplesLL.get(), dataLen * sizeof(float));
    samplesLL = std::move(newBuf);

    newBuf.reset(new float[bufferLen]);
    if (samplesLR)
        memcpy(newBuf.get(), samplesLR.get(), dataLen * sizeof(float));
    samplesLR = std::move(newBuf);

    newBuf.reset(new float[bufferLen]);
    if (samplesRL)
        memcpy(newBuf.get(), samplesRL.get(), dataLen * sizeof(float));
    samplesRL = std::move(newBuf);

    newBuf.reset(new float[bufferLen]);
    if (samplesRR)
        memcpy(newBuf.get(), samplesRR.get(), dataLen * sizeof(float));
    samplesRR = std::move(newBuf);
}
                                   ...
                                   float *in, float *out, int len)
{
    ...
    while (posOut < len)
    {
        std::unique_ptr<float[]> tmpIn(new float[flen]);
        std::unique_ptr<float[]> tmpOut(new float[flen]);
        ...
        hcPutSingle(hcSingle, tmpIn.get());
        hcProcessSingle(hcSingle);
        hcGetSingle(hcSingle, tmpOut.get());
